...
```{r}
df4<-df3
df3 <- subset(df4, state == "CA" | state == "TX"|state == "NY"| state == "FL"| state == "IL" )
tail(df3,10)
```
...
```{r}
qplot(df3$Price,xlab = 'Price',main = 'Price Hist',col ='red',bins = 30) # price
```
...
###Avg Area Income
```{r}
qplot(x=df3$AvgIncome,xlab = 'Avg Area Income',bins = 20,col ='red')
plot(x=df3$AvgIncome,y=df3$Price,xlab = 'Avg Area Income',ylab='Price',main = 'Price v Avg Area Income',col ='red')
abline(lm(Price~AvgIncome,data = df3),col= "Blue")
```
60k-80k
...
plot(x=df3$AvgHouseAge,y=df3$Price,xlab = 'Avg House Age',ylab='Price',main = 'Price v Avg House Age',col ='purple')
abline(lm(Price~AvgHouseAge,data = df3),col= "Blue")
```
5-7 years old
...
plot(x=df3$AvgNumRooms,y=df3$Price,xlab = 'Avg Num Rooms',ylab='Price',main = 'Price v Avg Num Rooms',col ='Black')
abline(lm(Price~AvgNumRooms,data = df3),col= "Blue")
```

###Average Number of Bedrooms
```{r}
qplot(df1$AvgNumBedrooms,xlab = 'Avg Num Bedrooms',main = 'Avg Num Bedrooms Hist',bins = 7,col ='red')
plot(x=df3$AvgNumBedrooms,y=df3$Price,xlab = 'Avg Num Bedrooms',ylab='Price',main = 'Price v Avg Num Bedrooms',col ='green')
abline(lm(Price~AvgNumBedrooms,data = df3),col= "Blue")
```
2-5
...
plot(x=df3$AreaPopulation,y=df3$Price,xlab = 'Area Population',ylab='Price',main = 'Price v Area Population',col ='red')
abline(lm(Price~AreaPopulation,data = df3),col= "Blue")
```
30k-45k
...
From AIC step function, avgnumbedrooms variable is removed. This is reasonable because by intuition, all features except average number of bedrooms are significant and it is correlated with the average number of rooms.
...
```{r}
poly_model <- lm(df_tr1$Price~polym(AvgIncome , AvgHouseAge , AvgNumRooms , AreaPopulation,degree=3,raw=TRUE),data=df_tr1)

rmse_r2_rss(df_tr$Price,predict(poly_model,df_tr),df_tr)
rmse_r2_rss(df_ts$Price,predict(poly_model,df_ts),df_ts)
...
train.control <- trainControl(method = "repeatedcv", 
                              number = 10, repeats = 3)
# Train the model
model_cv <- train(df_tr1$Price~polym(AvgIncome , AvgHouseAge , AvgNumRooms , AreaPopulation,degree=3,raw=TRUE), data=df3, method="lm", trControl = train.control)
