...
```{r}
qplot(df4$Price,xlab = 'Price',main = 'Price Hist',col ='red',bins = 30) # price
quantile(df4$Price, 0.75)
```
...
###Avg Area Income
```{r}
qplot(x=df4$AvgIncome,xlab = 'Avg Area Income',main="Avg Area Income Hist",bins = 20,col ='red')
plot(x=df4$AvgIncome,y=df4$Price,xlab = 'Avg Area Income',ylab='Price',main = 'Price v Avg Area Income',col ='red')
abline(lm(Price~AvgIncome,data = df4),col= "Blue")
quantile(df4$AvgIncome, 0.25)
```
60k-80k
...
dt = sort(sample(nrow(df4), nrow(df4)*.7))

#train
df_tr <- df4[dt,-7] #remove state
...
The adjusted R^2 is about $0.917$, which is close to 1.
...
```{r}
poly_model1 <- lm(df_tr1$Price~polym(AvgIncome , AvgHouseAge , AvgNumRooms , AreaPopulation,degree=4,raw=TRUE),data=df_tr1)

rmse_r2_rss(df_tr$Price,predict(poly_model1,df_tr),df_tr)
rmse_r2_rss(df_ts$Price,predict(poly_model1,df_ts),df_ts)

#############################################################
poly_model2 <- lm(df_tr1$Price~polym(AvgIncome , AvgHouseAge , AvgNumRooms , AreaPopulation,degree=3,raw=TRUE),data=df_tr1)

rmse_r2_rss(df_tr$Price,predict(poly_model2,df_tr),df_tr)
rmse_r2_rss(df_ts$Price,predict(poly_model2,df_ts),df_ts)
#############################################################
poly_model3 <- lm(df_tr1$Price~polym(AvgIncome , AvgHouseAge , AvgNumRooms , AreaPopulation,degree=2,raw=TRUE),data=df_tr1)
